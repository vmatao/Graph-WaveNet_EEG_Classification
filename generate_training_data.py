...
def cut_windows(df, before_event, after_event):
    ...
    for i in range(0, len(idx)):
        final_df.loc[i] = [idx[i], events[i]]

        # if indexa == 1:
    final_df.to_csv("A01Tie.csv", index=False)
    # else:
    #     final_df.to_csv("A02Tie.csv", index=False)
...
def prep_df(raw):
    ...
    # TODO some datasets have no events
...
def generate_train_val_test(args):
    ...
    for cat in ["train", "val", "test"]:
        ...
        np.savez_compressed(
            os.path.join(args.output_dir, f"{cat}ww.npz"),
            x=_x,
            y=_y,
